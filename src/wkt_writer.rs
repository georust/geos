use crate::{GContextHandle, Geometry, GResult, AsRaw, ContextHandling, ContextInteractions, OutputDimension};
...
impl<'a> WKTWriter<'a> {
    ...
    pub fn new() -> GResult<WKTWriter<'a>> {
        match GContextHandle::init_e(Some("WKTWriter::new")) {
    ...
    /// assert_eq!(writer.write(&point_geom).unwrap(), "POINT (2.5000000000000000 2.5000000000000000)");
    /// ```
    pub fn new_with_context(context: Arc<GContextHandle<'a>>) -> GResult<WKTWriter<'a>> {
        unsafe {
...
impl<'a> ContextInteractions<'a> for WKTWriter<'a> {
    ...
    ///
    /// ```
    /// use geos::{ContextInteractions, GContextHandle, WKTWriter};
    ///
