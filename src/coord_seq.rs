...
use crate::{
    AsRaw, ContextHandle, ContextHandling, ContextInteractions, CoordDimensions, Geometry, Ordinate,
};
...
impl<'a> CoordSeq<'a> {
    ...
    pub fn set_x(&mut self, line: usize, val: f64) -> GResult<()> {
        ...
        let ret_val =
            unsafe { GEOSCoordSeq_setX_r(self.get_raw_context(), self.as_raw(), line as _, val) };
        if ret_val == 0 {
    ...
    pub fn set_y(&mut self, line: usize, val: f64) -> GResult<()> {
        ...
        let ret_val =
            unsafe { GEOSCoordSeq_setY_r(self.get_raw_context(), self.as_raw(), line as _, val) };
        if ret_val == 0 {
    ...
    pub fn set_z(&mut self, line: usize, val: f64) -> GResult<()> {
        ...
        let ret_val =
            unsafe { GEOSCoordSeq_setZ_r(self.get_raw_context(), self.as_raw(), line as _, val) };
        if ret_val == 0 {
    ...
    /// let mut coords = CoordSeq::new(1, CoordDimensions::ThreeD)
    ///                           .expect("failed to create CoordSeq");
    /// coords.set_ordinate(0, Ordinate::Z, 10.).expect("failed to set ordinate");
    /// assert_eq!(coords.get_z(0), Ok(10.));
